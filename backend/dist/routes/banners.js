...
router.get("/", async (req, res) => {
    try {
        ...
        console.log("[BANNERS ROUTE] Supabase client config:", database_1.supabase);
        // Try a direct query to another table
        const { data: storesTest, error: storesError } = await database_1.supabase.from("stores").select("*").limit(1);
...
router.post("/", async (req, res) => {
    try {
        ...
        const { data, error } = await database_1.supabase.from("banners").insert([banner]).select();
        if (error)
...
router.put("/:id", async (req, res) => {
    ...
    try {
        const { data, error } = await database_1.supabase.from("banners").update(updates).eq("id", id).select();
        if (error)
            return res.status(500).json({ success: false, error: error.message });
        res.json({ success: true, data });
    }
...
router.delete("/:id", async (req, res) => {
    const { id } = req.params;
    try {
        const { error } = await database_1.supabase.from("banners").delete().eq("id", id);
        if (error) {
            return res.status(500).json({ success: false, error: error.message });
        }
