...
router.get("/", async (req, res) => {
    try {
        let query = database_1.supabase.from("stores").select("*");
...
router.post("/", async (req, res) => {
    try {
        const { name, description, email, phone, address, city_id, // required
        slug, // required
        location_lat, location_lng, delivery_range, delivery_fee, min_order_amount, theme_color, opening_hours, social_media, settings, } = req.body;
...
router.put("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const { name, description, email, phone, address, city_id, slug, location_lat, location_lng, delivery_range, delivery_fee, min_order_amount, theme_color, is_active, is_featured, opening_hours, social_media, settings, } = req.body;
